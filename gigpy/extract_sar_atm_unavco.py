...
INTRODUCTION = '''
    Extract the SAR synchronous GPS tropospheric products from UNAVCO.
'''

EXAMPLE = '''EXAMPLES:

    extract_sar_atm_unavco.py search_gps.txt imaging_time --parallel 4
    extract_sar_atm_unavco.py search_gps.txt imaging_time --date 20180101 
    extract_sar_atm_unavco.py search_gps.txt imaging_time --date-txt date_list.txt
...
def main(argv):
    ...
    # Start to extract data using parallel process
    ut.parallel_process(data_parallel, extract_sar_pwv, n_jobs=inps.parallelNumb, use_kwargs=False)
